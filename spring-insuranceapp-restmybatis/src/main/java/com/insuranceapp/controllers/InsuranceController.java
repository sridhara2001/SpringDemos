...
//		create headers
		...
		// get data for the body of response
	...
	ResponseEntity<List<Insurance>> getByTypeAndLessPremium(@PathVariable("type") String type, @PathVariable("premium") double premium){
		List<Insurance> insurances = insuranceService.getByTypeAndLessPremium(type, premium);
		HttpHeaders headers = new HttpHeaders();
		headers.add("info", "returning list of insurances by type and premium"+type+" and "+premium);
